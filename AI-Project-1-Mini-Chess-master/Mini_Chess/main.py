...
# Game Setup
WINDOW_WIDTH = 480  # Increased to accommodate 6x6 board
WINDOW_HEIGHT = 700
SQ_SIZE = 80
DIMENSION_X = 6  # Changed from 5 to 6
DIMENSION_Y = 6
FPS = 60
...
BUTTON_WIDTH = 50
BUTTON_HEIGHT = 50
PLAY_BUTTON_POS = (410, 500)  # Adjusted for wider window
RESTART_BUTTON_POS = (410, 560)  # Adjusted for wider window
TOGGLE_BUTTON_1_POS = (280, 500)  # Adjusted for wider window
TOGGLE_BUTTON_2_POS = (210, 500)  # Adjusted for wider window
TOGGLE_BUTTON_3_POS = (280, 560)  # Adjusted for wider window
TOGGLE_BUTTON_4_POS = (210, 560)  # Adjusted for wider window
...
def loadSoundEffects():
    ...
    effects = {}
    move_piece = pygame.mixer.Sound('C:/Users/91966/Downloads/AI-Project-1-Mini-Chess-master/AI-Project-1-Mini-Chess-master/Mini_Chess/audios/move_pieces.wav')
    undo_move = pygame.mixer.Sound('C:/Users/91966/Downloads/AI-Project-1-Mini-Chess-master/AI-Project-1-Mini-Chess-master/Mini_Chess/audios/undo_moves.wav')
    checkmate = pygame.mixer.Sound('C:/Users/91966/Downloads/AI-Project-1-Mini-Chess-master/AI-Project-1-Mini-Chess-master/Mini_Chess/audios/checkmate_sound.wav')
...
def loadImages():
    ...
    for piece in pieces:
        image = pygame.image.load('C:/Users/91966/Downloads/AI-Project-1-Mini-Chess-master/AI-Project-1-Mini-Chess-master/Mini_Chess/images/' + piece + '.png')
    ...
    for row in range(0, DIMENSION_Y):
        for col in range(0, DIMENSION_X):
            ...
            if col == 0:
                ...
                WINDOW.blit(surface, (7, row * SQ_SIZE + 7))

            # Render file value in the bottom row
            if row == DIMENSION_Y - 1:
                font = pygame.font.SysFont('Comic Sans', 18)
                surface = font.render(file_, True, 'blue')
                WINDOW.blit(surface, (col * SQ_SIZE + 70,
                            (DIMENSION_Y-1) * SQ_SIZE + 60))
...
def drawPieces(WINDOW, Board):
    ...
    for row in range(DIMENSION_Y):  # Changed from DIMENSION_X to DIMENSION_Y
        for col in range(DIMENSION_X):  # Changed from DIMENSION_Y to DIMENSION_X
            piece = Board[row][col]
            if piece != '--':
...
def drawButtons(WINDOW, GAME_STATE):
    ...
    # # ==> Show Text (Opponent)
    drawTextMessage(WINDOW, "Black", [20, 510], pygame.Color('darkmagenta'))
    drawTextMessage(WINDOW, "White", [20, 570], pygame.Color('darkmagenta'))
    drawTextMessage(WINDOW, "MoveCount", [
                    20, 630], pygame.Color('darkmagenta'))
    drawTextMessage(WINDOW, MOVE_COUNT, [150, 630], pygame.Color('red'))
    drawUIStatus(WINDOW, GAME_STATE)

    # ==> Restart Button

    icon_image = pygame.image.load('C:/Users/91966/Downloads/AI-Project-1-Mini-Chess-master/AI-Project-1-Mini-Chess-master/Mini_Chess/icons/re2.png')
    ...
    icon_image = pygame.image.load('C:/Users/91966/Downloads/AI-Project-1-Mini-Chess-master/AI-Project-1-Mini-Chess-master/Mini_Chess/icons/play2.png')
...
def drawCheckText(screen, text):
    font = pygame.font.SysFont("Helvetica", 38, True, False)
...
def drawGameOverText(screen, text, textColor):
    font = pygame.font.SysFont("Helvetica", 22, True, False)
    textObject = font.render(text, 0, pygame.Color(textColor))
    textLocation = pygame.Rect(0, 0, WINDOW_WIDTH, WINDOW_HEIGHT).move(
        WINDOW_WIDTH/2 - textObject.get_width()/2, 665)
    screen.blit(textObject, textLocation)
...
def drawUIStatus(WINDOW, GAME_STATE):
    ...
    if BLACK_AI == BLACK_MAN == WHITE_AI == WHITE_MAN == False:
        ...
        drawTextMessage(WINDOW, turn, [20, 665], pygame.Color('olivedrab4'))

    elif GAME_STATE.checkMate or GAME_STATE.staleMate:
        drawTextMessage(WINDOW, '', [100, 665], pygame.Color('white'))

    elif (BLACK_AI or BLACK_MAN) and (WHITE_AI or WHITE_MAN) and not GAME_STARTED:
        turn = 'Click Play to Start!'
        drawTextMessage(WINDOW, turn, [100, 665], pygame.Color('olivedrab4'))
        ...
        drawTextMessage(WINDOW, turn, [120, 665], pygame.Color('olivedrab4'))
...
def main():
    ...
    board_rect = pygame.Rect(
        0, 0, DIMENSION_X * SQ_SIZE, DIMENSION_Y * SQ_SIZE)  # Updated to include all 6 columns
